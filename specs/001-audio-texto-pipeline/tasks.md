...
- [X] T001 Criar estrutura de diretórios `src/audio/`, `tests/audio/contract/`, `tests/audio/integration/`, `tests/audio/unit/`, `data/raw/coswara/`, `data/raw/pt-br/`, `data/audio/processed/`, `data/audio/reports/` com `.gitkeep` nos diretórios `data/` (não versionar áudio real)
...
- [X] T005 [P] Configurar `ruff` e `pyright` em `pyproject.toml` com target Python 3.14, perfil PEP 8 e regras de tipos estritas para `src/audio/`
...
- [X] T006 Criar `src/audio/audio_schemas.py` com todos os modelos Pydantic v2 conforme `data-model.md`: `AudioProcessingRequest` (patient_id, audio_path, language, source, max_duration_seconds, max_size_mb, `timeout_seconds` default `60` — NFR-001), `AudioMetadata` (duration_seconds, size_mb, format, sample_rate_hz, channels), `TranscriptionResult` (text, confidence, language, provider, status enum), `AcousticFeatures` (rms, peak_amplitude, silence_ratio, long_pause_count, cough_or_breathing_indicator, score_acustico, signals), `TextFindings` (critical_terms, sentiment, sentiment_confidence, score_textual, score_textual_efetivo), `AudioAlert` (patient_id, modulo literal "audio_texto", tipo_anomalia, score_risco, nivel_risco, descricao, recomendacao, evidencias com `duracao_processamento_ms` e `excedeu_limite_latencia` — SC-007) com todas as validações de faixa `[0.0, 1.0]` e enumerações
- [X] T007 [P] Adicionar exceções customizadas do módulo em `src/audio/audio_schemas.py`: classe base `AudioPipelineError` com campo `error_code` suportando os códigos `invalid_patient_id`, `unsupported_format`, `file_not_found`, `file_too_large`, `duration_too_long`, `corrupted_audio`, `missing_azure_configuration`
- [X] T008 [P] Criar `tests/audio/contract/test_audio_input_contract.py` para validar `AudioProcessingRequest` contra `specs/001-audio-texto-pipeline/contracts/audio-input.schema.json`: patient_id obrigatório, formatos WAV/MP3, limites de tamanho e duração
- [X] T009 [P] Criar `tests/audio/contract/test_audio_alert_contract.py` para validar `AudioAlert` contra `specs/001-audio-texto-pipeline/contracts/audio-alert.schema.json`: modulo fixo "audio_texto", score_risco ∈ [0.0, 1.0], nivel_risco coerente com score (baixo < 0.4, moderado ≥ 0.4, alto ≥ 0.7), todos os 7 campos obrigatórios presentes
- [X] T010 Criar `src/audio/audio_storage.py` com funções `save_processed(patient_id, data, processed_dir)` e `save_report(alert: AudioAlert, output_dir)` escrevendo JSON em `data/audio/processed/` e `data/audio/reports/` respectivamente; logs de storage devem incluir apenas patient_id (como referência) e caminho do arquivo, sem conteúdo clínico

**Checkpoint**: `uv run pytest tests/audio/contract/` deve passar — contratos e schemas validados
...
- [X] T018 [P] [US1] Criar `src/audio/audio_cli.py` com CLI Typer: comando `process` com opções `--patient-id` (obrigatório), `--audio-path` (obrigatório), `--language` (default `pt-BR`), `--output-dir` (default `data/audio/reports`), `--processed-dir` (default `data/audio/processed`), `--max-duration-seconds` (default `600`), `--max-size-mb` (default `50`); carregar `.env` via `python-dotenv`; em sucesso exibir apenas caminho do relatório e status; em erro retornar exit code não-zero com categoria de erro (conforme contracts/cli.md); nunca exibir credenciais, transcrição ou termos clínicos

**Checkpoint Phase 3**: `uv run pytest tests/` deve passar. `uv run python -m src.audio.audio_cli process --patient-id sample_001 --audio-path data/raw/pt-br/sample.wav` deve gerar JSON em `data/audio/reports/` com todos os 7 campos obrigatórios.
...
**Independent Test**: Fornecer gravação com fala clara → TranscriptionResult com status success e confiança alta; fornecer gravação com ruído → TranscriptionResult com status low_confidence; verificar que transcrição é salva em `data/audio/processed/` e não aparece em logs

### Implementation para US2

- [X] T019 [P] [US2] Criar `tests/audio/unit/test_audio_azure_adapters.py` com mocks do Azure Speech SDK (`pytest-mock`): sucesso com confiança alta (status `success`), confiança abaixo do threshold (status `low_confidence`), sem fala detectada (status `no_speech`), falha de serviço (status `failed`), normalização de confiança de escala alternativa para `[0.0, 1.0]`
- [X] T020 [US2] Adicionar `save_transcription(patient_id, transcription_result, processed_dir)` em `src/audio/audio_storage.py` persistindo `TranscriptionResult` serializado em `data/audio/processed/` para rastreabilidade/auditoria (FR-017); garantir que o texto da transcrição não aparece em nenhum log do pipeline

**Checkpoint Phase 4**: `uv run pytest tests/audio/unit/test_audio_azure_adapters.py` deve cobrir todos os status de transcrição. Arquivo em `data/audio/processed/` gerado após processamento.
...
**Checkpoint Phase 6**: `uv run pytest tests/audio/unit/test_audio_azure_adapters.py` deve cobrir casos de termos críticos e sentimento.
...
- [X] T030 [P] [US1] Criar `tests/audio/unit/test_audio_latency.py` cobrindo: processamento dentro do `timeout_seconds` configurado → `excedeu_limite_latencia = False` e nenhum log de falha de performance; processamento simulado (via mock de tempo) que excede o limite → `excedeu_limite_latencia = True`, log de falha de performance emitido e `AudioAlert` ainda gerado normalmente; alteração de `timeout_seconds` via parâmetro de configuração sem qualquer mudança de código (NFR-001)
- [X] T031 [P] Criar `tests/audio/integration/test_audio_batch_sample_validation.py` (ou script equivalente em `tests/audio/integration/`) que processa um conjunto de amostras conhecidas sem queixas relevantes e sem sinais acústicos de risco (ex.: 10–20 amostras PT-BR/Coswara "healthy" do subconjunto de demo) e valida que ao menos 90% resultam em `nivel_risco = "baixo"` e `tipo_anomalia = "nenhuma"`, conforme SC-006; documentar no teste a origem/composição da amostra usada
- [X] T032 [P] [US1] Estender `tests/audio/unit/test_audio_pipeline.py` (ou criar `tests/audio/unit/test_audio_input_validation.py`) com fixtures reais cobrindo FR-015 de ponta a ponta via `process_audio_recording`: arquivo WAV vazio/com bytes corrompidos → `AudioPipelineError(corrupted_audio)`; arquivo cujo tamanho em disco excede `max_size_mb` → `AudioPipelineError(file_too_large)`; arquivo cuja duração real (via `AudioMetadata.duration_seconds`) excede `max_duration_seconds` → `AudioPipelineError(duration_too_long)`; em todos os casos, nenhum `AudioAlert` deve ser gerado nem persistido em `data/audio/reports/`
- [X] T033 [P] Criar `tests/audio/unit/test_audio_observability.py` validando as 3 métricas mínimas obrigatórias de NFR-002 emitidas por `src/audio/audio_pipeline.py`: log estruturado contém latência total (`duracao_processamento_ms`), indicação de rejeição de entrada (taxa de rejeição observável a partir do status/`error_code` logado) e o `score_risco` gerado; validar também que nenhum log contém áudio bruto, transcrição ou termos críticos (reforço de VII/VIII)
- [X] T034 [P] Criar `tests/audio/unit/test_audio_module_boundary.py` (opcional, reforça FR-016 e Princípio III da constituição) verificando por inspeção estática de imports que `src/audio/audio_pipeline.py`, `src/audio/audio_azure.py`, `src/audio/audio_acoustics.py`, `src/audio/audio_scoring.py` e `src/audio/audio_storage.py` não importam módulos de outras modalidades (`video_pipeline`, `text_pipeline`, `anomaly`, `alerts`, `fusion`)

**Checkpoint Phase 7**: `uv run pytest tests/audio/unit/test_audio_latency.py tests/audio/integration/test_audio_batch_sample_validation.py tests/audio/unit/test_audio_observability.py tests/audio/unit/test_audio_module_boundary.py` deve passar, cobrindo NFR-001, NFR-002, FR-015, FR-016, SC-006 e SC-007.
...
Task T011: "Criar tests/audio/unit/test_audio_scoring.py"
...
Task T012: "Criar tests/audio/unit/test_audio_pipeline.py"
```
