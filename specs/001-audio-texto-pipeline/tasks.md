...
**Depende de**: Phase 3 (US1) completa — usa `src/audio_pipeline.py`, `src/audio_scoring.py` e o subconjunto Coswara/PT-BR já preparados

- [ ] T029 [US1] Adicionar medição de latência total em `src/audio_pipeline.py`: capturar timestamp de início/fim do pipeline, calcular `duracao_processamento_ms`, comparar com `request.timeout_seconds` (default `60`, NFR-001) e preencher `AudioAlert.evidencias.excedeu_limite_latencia = True` quando excedido; **não** abortar o processamento nem lançar exceção — apenas registrar a violação como falha de performance observável (SC-007) via log estruturado (ex.: `status="performance_timeout"`) além do campo em `evidencias`
- [ ] T030 [P] [US1] Criar `tests/unit/test_audio_latency.py` cobrindo: processamento dentro do `timeout_seconds` configurado → `excedeu_limite_latencia = False` e nenhum log de falha de performance; processamento simulado (via mock de tempo) que excede o limite → `excedeu_limite_latencia = True`, log de falha de performance emitido e `AudioAlert` ainda gerado normalmente; alteração de `timeout_seconds` via parâmetro de configuração sem qualquer mudança de código (NFR-001)
- [ ] T031 [P] Criar `tests/integration/test_audio_batch_sample_validation.py` (ou script equivalente em `tests/integration/`) que processa um conjunto de amostras conhecidas sem queixas relevantes e sem sinais acústicos de risco (ex.: 10–20 amostras PT-BR/Coswara "healthy" do subconjunto de demo) e valida que ao menos 90% resultam em `nivel_risco = "baixo"` e `tipo_anomalia = "nenhuma"`, conforme SC-006; documentar no teste a origem/composição da amostra usada
- [ ] T032 [P] [US1] Estender `tests/unit/test_audio_pipeline.py` (ou criar `tests/unit/test_audio_input_validation.py`) com fixtures reais cobrindo FR-015 de ponta a ponta via `process_audio_recording`: arquivo WAV vazio/com bytes corrompidos → `AudioPipelineError(corrupted_audio)`; arquivo cujo tamanho em disco excede `max_size_mb` → `AudioPipelineError(file_too_large)`; arquivo cuja duração real (via `AudioMetadata.duration_seconds`) excede `max_duration_seconds` → `AudioPipelineError(duration_too_long)`; em todos os casos, nenhum `AudioAlert` deve ser gerado nem persistido em `data/reports/`
- [ ] T033 [P] Criar `tests/unit/test_audio_observability.py` validando as 3 métricas mínimas obrigatórias de NFR-002 emitidas por `src/audio_pipeline.py`: log estruturado contém latência total (`duracao_processamento_ms`), indicação de rejeição de entrada (taxa de rejeição observável a partir do status/`error_code` logado) e o `score_risco` gerado; validar também que nenhum log contém áudio bruto, transcrição ou termos críticos (reforço de VII/VIII)
- [ ] T034 [P] Criar `tests/unit/test_audio_module_boundary.py` (opcional, reforça FR-016 e Princípio III da constituição) verificando por inspeção estática de imports que `src/audio_pipeline.py`, `src/audio_azure.py`, `src/audio_acoustics.py`, `src/audio_scoring.py` e `src/audio_storage.py` não importam módulos de outras modalidades (`video_pipeline`, `text_pipeline`, `anomaly`, `alerts`, `fusion`)
...
- [ ] T035 [P] Executar `uv run ruff check .` e corrigir todos os avisos em `src/` e `tests/` (Princípio IV)
- [ ] T036 [P] Executar `uv run pyright` e corrigir erros de tipo em todas as interfaces públicas de `src/audio_pipeline.py`, `src/audio_schemas.py`, `src/audio_azure.py`, `src/audio_scoring.py` e `src/audio_acoustics.py`
- [ ] T037 Adicionar docstrings nas interfaces públicas de `src/audio_pipeline.py` (`process_audio_recording`), `src/audio_schemas.py` (classes principais), `src/audio_azure.py` e `src/audio_scoring.py` explicando contratos e invariantes (Princípio X)
