...
- Os limiares de `nivel_risco` definidos nesta spec (baixo < 0.4, moderado 0.4–0.7, alto ≥ 0.7) são específicos do alerta do módulo `audio_texto` e não são necessariamente os mesmos limiares usados pela camada de fusão multimodal para calcular um `risco_final` agregado (que pode adotar sua própria calibração, ex.: outra fronteira entre "moderado" e "alto"). Cada módulo (`audio_texto`, `video_fisioterapia`, `anomalias_clinicas_uti`) classifica seu próprio `nivel_risco` de forma independente; a camada de fusão não deve assumir que os limiares dos módulos de origem são idênticos aos seus próprios limiares de agregação.
