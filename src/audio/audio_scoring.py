...
# Termos equivalentes em inglês para datasets como deepgram-diarized-272.
CRITICAL_TERMS_EN: tuple[str, ...] = (
    "shortness of breath",
    "short of breath",
    "chest pain",
    "chest tightness",
...
# Mapa de idioma → termos. Fallback para PT quando idioma não mapeado.
_CRITICAL_TERMS_BY_LANG: dict[str, tuple[str, ...]] = {
    "pt-BR": CRITICAL_TERMS_PT,
    "pt":    CRITICAL_TERMS_PT,
    "en-US": CRITICAL_TERMS_EN,
    "en-GB": CRITICAL_TERMS_EN,
    "en":    CRITICAL_TERMS_EN,
}

# Alias de retrocompatibilidade — código existente que importa CRITICAL_TERMS continua funcionando.
CRITICAL_TERMS = CRITICAL_TERMS_PT
...
def find_critical_terms(text: str, language: str = "pt-BR") -> list[str]:
    """Retorna termos críticos distintos encontrados no texto.

    Seleciona a lista de termos pelo idioma. Idiomas não mapeados fazem
    fallback para PT-BR.
    """
    terms = _CRITICAL_TERMS_BY_LANG.get(language, CRITICAL_TERMS_PT)
    lowered = text.lower()
    found: list[str] = []
    for term in terms:
